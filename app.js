...
index e1d8c66204aa316cd764cc533242796b60fb5148..941885965a4f949b88f7d01d5974314de0de06c0 100644
--- a/app.js
+++ b/app.js
@@ -1,63 +1,74 @@
...
@@ -75,169 +86,224 @@ const nextSlideBtn = document.querySelector('#next-slide-btn');
...
+  if (data) return data;
+  const { data: inserted, error: insertError } = await supabaseClient.from('profiles').insert({ id: currentUser.id }).select('*').maybeSingle();
+  if (insertError) throw insertError;
+  return inserted || { id: currentUser.id, onboarding_complete: false };
 }
 
-function emptySlideResponses() { return Object.fromEntries(HUB_SLIDES.map((s) => [s.key, { input: '', extras: {}, images: [] }])); }
+function isOnboardingComplete(profile) {
+  if (!profile) return false;
+  if (typeof profile.onboarding_complete === 'boolean') return profile.onboarding_complete;
...
+  const payload = {};
+  if (Object.hasOwn(currentProfile || {}, 'onboarding_completed')) payload.onboarding_completed = true;
+  else payload.onboarding_complete = true;
+  const { data, error } = await supabaseClient.from('profiles').update(payload).eq('id', currentUser.id).select('*').maybeSingle();
...
+  const attemptIsActive = await supabaseClient.from('cycles').select('*').eq('space_id', spaceId).eq('is_active', true).maybeSingle();
+  if (!attemptIsActive.error && attemptIsActive.data) return attemptIsActive.data;
+  const attemptStatus = await supabaseClient.from('cycles').select('*').eq('space_id', spaceId).eq('status', 'active').maybeSingle();
+  if (attemptStatus.error) throw attemptStatus.error;
+  return attemptStatus.data || null;
+}
+
...
+  const bySlug = await supabaseClient.from('spaces').select('*').eq('slug', INDIVIDUAL_SPACE_SLUG).maybeSingle();
+  const byType = await supabaseClient.from('spaces').select('*').eq('type', 'individual').maybeSingle();
+  const byMode = await supabaseClient.from('spaces').select('*').eq('mode', 'individual').maybeSingle();
+  const byName = await supabaseClient.from('spaces').select('*').ilike('name', '%individual%').limit(1).maybeSingle();
+  if (bySlug.error && byType.error && byMode.error && byName.error) throw bySlug.error;
+  individualSpace = bySlug.data || byType.data || byMode.data || byName.data;
...
@@ -248,298 +314,286 @@ function renderSlide() {
...
+      if (signInResult.error && !String(signInResult.error.message || '').toLowerCase().includes('email')) {
+        throw signInResult.error;
+      }
+      if (signInResult.error) {
+        authError.textContent = 'Account created. Please verify your email, then sign in.';
+      }
+    } else {
...
-  renderClubDashboard();
-});
+  if (!isOnboardingComplete(currentProfile)) await markOnboardingComplete();
...
+    } catch (error) {
+      authError.textContent = error.message || 'Unable to load account.';
+      hideAllMainScreens();
+      signupScreen.classList.remove('hidden');
+    }
+  });
